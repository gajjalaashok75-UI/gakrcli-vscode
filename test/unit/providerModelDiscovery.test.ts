...
describe('providerModelDiscovery', () => {
  ...
  it('fetches model IDs from an OpenAI-compatible /v1/models endpoint', async () => {
  ...
  it('returns empty when API request fails', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: false,
      json: vi.fn(),
    });
  ...
  it('returns empty when fetch is not available', async () => {
  ...
  it('handles string-array model responses', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      json: vi.fn().mockResolvedValue({
        data: ['model-one', 'model-two', 'model-one'],
      }),
    });
    globalThis.fetch = fetchMock as unknown as typeof fetch;
    ...
    await expect(discoverOpenAICompatibleModelOptions({
      ...
      { value: 'model-one', displayName: 'model-one' },
      { value: 'model-two', displayName: 'model-two' },
    ]);
  });
});
