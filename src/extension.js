...
const { ChatController, gakrcliChatViewProvider, gakrcliChatPanelManager } = require('./chat/chatProvider');
...
async function collectControlCenterState() {
  ...
  const terminalName = configured.get('terminalName', 'gakrCLI');
...
async function launchGakrcli(options = {}) {
  ...
  if (!installed) {
    const action = await vscode.window.showErrorMessage(
      `gakrcli command not found: ${executable}. Install it with: npm install -g @gakr-gakr/gakrcli`,
      'Open Setup Guide',
      'Open Repository',
    );

    if (action === 'Open Setup Guide') {
      await vscode.env.openExternal(vscode.Uri.parse(GAKRCLI_SETUP_URL));
    } else if (action === 'Open Repository') {
      await vscode.env.openExternal(vscode.Uri.parse(GAKRCLI_REPO_URL));
    }
...
function getWorkspaceRootActionDetail(status, fallbackDetail) {
  ...
  if (status.launchActionsShareTargetReason === 'relative-launch-command') {
    return `Same workspace-root target as Launch GakrCLI because the relative command resolves from the workspace root · ${status.workspaceRootCwdLabel}`;
  }
      ...
      <header class="hero">
        <div class="hero-top">
          <div class="brand">
            ...
            <div class="wordmark" aria-label="GakrCLI wordmark"><span class="wordmark-accent">GakrCLI</span></div>
            <div class="headline">
        ...
        <section class="support-card" aria-labelledby="quick-links-title">
          ...
          <div class="support-stack">
            ...
            <button class="support-link" id="repo" type="button">
              ...
              <span class="summary-detail">Browse the upstream GakrCLI project.</span>
            </button>
            <button class="support-link" id="commands" type="button">
              <span class="support-link-label">Open Command Palette</span>
              <span class="summary-detail">Access VS Code and GakrCLI commands quickly.</span>
            </button>
          </div>
        </section>
      </section>
...
class gakrcliControlCenterProvider {
  constructor() {
    ...
    webviewView.webview.onDidReceiveMessage(async message => {
      switch (message?.type) {
        case 'launch':
          ...
          break;
        case 'launchRoot':
          await launchGakrcli({ requireWorkspace: true });
          break;
        case 'openProfile':
...
function activate(context) {
  ...
  const chatViewProvider = new gakrcliChatViewProvider(chatController);
  const chatPanelManager = new gakrcliChatPanelManager(chatController);
  ...
  statusBarItem.text = '$(comment-discussion) GakrCLI';
  statusBarItem.tooltip = 'Open GakrCLI Chat';
  statusBarItem.command = 'gakrcli.openChat';
  statusBarItem.show();

  chatController.onDidChangeState((state) => {
    switch (state) {
      case 'streaming':
        statusBarItem.text = '$(sync~spin) GakrCLI';
        statusBarItem.tooltip = 'GakrCLI is generating...';
        break;
      case 'connected':
        statusBarItem.text = '$(comment-discussion) GakrCLI';
        statusBarItem.tooltip = 'GakrCLI connected';
        break;
      default:
        statusBarItem.text = '$(comment-discussion) GakrCLI';
        statusBarItem.tooltip = 'Open GakrCLI Chat';
  ...
  // ── Existing commands ──
  const startCommand = vscode.commands.registerCommand('gakrcli.start', async () => {
    await launchGakrcli();
  });

  const startInWorkspaceRootCommand = vscode.commands.registerCommand(
    'gakrcli.startInWorkspaceRoot',
    async () => {
      await launchGakrcli({ requireWorkspace: true });
    },
  );

  const openDocsCommand = vscode.commands.registerCommand('gakrcli.openDocs', async () => {
    await vscode.env.openExternal(vscode.Uri.parse(GAKRCLI_REPO_URL));
  });

  const openSetupDocsCommand = vscode.commands.registerCommand(
    'gakrcli.openSetupDocs',
    async () => {
      await vscode.env.openExternal(vscode.Uri.parse(GAKRCLI_SETUP_URL));
    },
  );
  ...
  const openWorkspaceProfileCommand = vscode.commands.registerCommand(
    'gakrcli.openWorkspaceProfile',
    async () => {
      await openWorkspaceProfile();
    },
  );

  const openUiCommand = vscode.commands.registerCommand('gakrcli.openControlCenter', async () => {
    await vscode.commands.executeCommand('workbench.view.extension.gakrcli');
  });

  // ── New chat commands ──
  const newChatCommand = vscode.commands.registerCommand('gakrcli.newChat', () => {
    chatController.stopSession();
    chatController.broadcast({ type: 'session_cleared' });
  });

  const openChatCommand = vscode.commands.registerCommand('gakrcli.openChat', () => {
    chatPanelManager.openPanel();
  });

  const resumeSessionCommand = vscode.commands.registerCommand('gakrcli.resumeSession', async () => {
  ...
  const abortChatCommand = vscode.commands.registerCommand('gakrcli.abortChat', () => {
    chatController.abort();
  });

  // ── Register providers ──
  const controlCenterProviderReg = vscode.window.registerWebviewViewProvider(
    'gakrcli.controlCenter',
    provider,
  );
  ...
  context.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration(event => {
      if (event.affectsConfiguration('gakrcli')) {
        refreshProvider();
      }
    }),
