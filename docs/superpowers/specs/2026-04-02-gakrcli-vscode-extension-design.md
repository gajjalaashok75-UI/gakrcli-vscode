...
GakrCLI is a fork of Claude Code that adds an OpenAI-compatible shim (786 lines, 6 files changed), enabling any LLM via the OpenAI Chat Completions API — GPT-4o, Gemini, DeepSeek, Ollama, and 200+ models. It has the same CLI interface, tools, and capabilities as Claude Code.
...
| `claude-code-settings.schema.json` | Claude Code extension (21KB, readable JSON) | Rebrand references from "claude" → "gakrcli" |
...
│  - Process manager (spawn/kill gakrcli CLI)  │
...
│  ↕ stdin/stdout (NDJSON)                        │
├─────────────────────────────────────────────────┤
│  Layer 3: GakrCLI CLI (child process)        │
...
- Provider-specific: `GAKR_USE_OPENAI=1`, `OPENAI_API_KEY`, `OPENAI_MODEL`, `OPENAI_BASE_URL`, etc.
...
| `gakrcliCode.gakrcliProcessWrapper` | string | — | Custom process wrapper |
...
2. "Open GakrCLI" — how to launch
3. "Chat with GakrCLI" — basic usage with @-mentions
...
> **Implementation note:** We fork the Claude Code settings schema, rebrand "claude" → "gakrcli", and contribute it in package.json. The CLI already validates these settings — the extension just provides IDE autocomplete and validation.
...
│   ├── gakrcli-logo.png
│   ├── gakrcli-logo.svg
│   ├── gakrcli-logo-done.svg
│   ├── gakrcli-logo-pending.svg
│   └── walkthrough/
│       ├── step1.md
│       ├── step2.md
│       ├── step3.md
...
- [ ] Click status bar → opens GakrCLI panel
